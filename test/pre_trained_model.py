...
class RawNetWithSA(nn.Module):
    def __init__(self, d_args, device):
        ...
        self.gru = nn.GRU(input_size = d_args['filts'][2][-1],
			...
			batch_first = True)
        self.gru1 = nn.GRU(input_size = d_args['filts'][2][-1],
			hidden_size = d_args['gru_node'],
			num_layers = d_args['nb_gru_layer'],
			batch_first = True)
    ...
    def forward(self, x, y = 0, nb_ta_samp = 32076, is_crop_aggregating=True, is_test=False, is_TS=False):
        #16038
        #24057
        #32076
        l_x = []
